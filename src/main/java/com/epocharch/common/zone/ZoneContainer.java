...
public class ZoneContainer {
    ...
    private ZoneContainer() {
        ...
        String tmplevel = ProperitesContainer.getInstance().getProperty(PropKey.DEPLOY_LEVEL);
    ...
    private void watchZoneChange() {
        try {
            String servers = ProperitesContainer.getInstance().getProperty(PropKey.ZK_SERVERS);
            ZkClient localZk = new ZkClient();

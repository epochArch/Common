...
public class PropertiesContainer {
	...
	private static PropertiesContainer pContainer = new PropertiesContainer();
		...
		try {
			if (filePath != null) {
				...
				if (file.exists()) {
					...
					input = loadFileFromClasspath(filePath);
				}
			} else {
				input = loadFileFromClasspath(filePath);
			}
	...
	/**
	 * Set and merge external config into PropertiesContainer.
	 * @param namespace
	 * @param properties
	 */
	private void setAndMergeProperties(String namespace, Properties properties){
		if(pMap.containsKey(namespace)){
			logger.info("PropertiesContainer already has this namespace:"+namespace);
			Properties p = pMap.get(namespace);
			p.putAll(properties);
		}else{
